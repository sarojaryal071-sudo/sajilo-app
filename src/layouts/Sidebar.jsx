      ...
      {availability === 'scheduled' && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {/* Calendar Button - NO INPUT, NO TYPING */}
            <div style={{ position: 'relative' }}>
              <button onClick={() => setShowSideCalendar(!showSideCalendar)} style={{
                ...
                {sidebarDate ? formatDate(sidebarDate) : '📅 Select date'}
              </button>
              {showSideCalendar && (
                ...
                {showHour && (
                  <div style={{
                    ...
                    {HOURS.map((h) => (
                      ...
                      style={{
                        padding: '6px 4px', fontSize: 13, cursor: 'pointer', textAlign: 'center',
                        background: selectedHour === h ? 'var(--accent-blue-light)' : 'transparent',
                        color: selectedHour === h ? 'var(--accent-blue)' : 'var(--text-primary)',
                      }}
                ...
                {showMinute && (
                  <div style={{
                    ...
                    {MINUTES.map((m) => (
                      ...
                      style={{
                        padding: '6px 4px', fontSize: 13, cursor: 'pointer', textAlign: 'center',
                        background: selectedMinute === m ? 'var(--accent-blue-light)' : 'transparent',
                        color: selectedMinute === m ? 'var(--accent-blue)' : 'var(--text-primary)',
                      }}
                      onMouseEnter={(e) => { e.target.style.background = 'var(--accent-blue-light)'; e.target.style.color = 'var(--accent-blue)'; }}
