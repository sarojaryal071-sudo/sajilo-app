    ...
    case "earningsHero": {
      ...
      return (
        <div style={{
          ...
          {/* Left side: total earnings + job count */}
          ...
          {/* Right side: compact financial KPIs (ledger‑based) */}
          {financialOverview && (
            <div style={{
              display: 'grid',
              ...
              {[
                { label: 'Invoiced', value: financialOverview.invoiced },
                { label: 'Collected', value: financialOverview.collected },
                { label: 'Commission', value: financialOverview.commission },
                { label: 'Net', value: financialOverview.net },
              ].map(item => (
                <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', gap: '8px' }}>
                  <span style={{ opacity: 0.75 }}>{item.label}</span>
                  <span style={{ fontWeight: 600 }}>
                    {item.value === 0 && item.label !== 'Net' ? '—' : `Rs ${item.value.toLocaleString()}`}
                  </span>
                </div>
              ))}
              {financialOverview.dues > 0 && (
                <div style={{
                  ...
                  <span style={{ fontWeight: 700, color: '#FFD700' }}>
                    Rs {financialOverview.dues.toLocaleString()}
                  </span>
                </div>
              )}
            </div>
