...
function CategoryButton({ service, isActive, onClick }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      fontSize: 'var(--font-body-sm)', fontWeight: 500, cursor: 'pointer',
...
function ServiceTile({ service, isSelected, onClick, cardStyle, iconStyle, labelStyle }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      cursor: 'pointer', border: isSelected ? '2px solid var(--accent-blue)' : cardStyle.border,
...
export default function HomeScreen({ navigate, t }) {
  ...
  const layoutDecision = React.useMemo(
    () => resolveClientHomeLayout(homepageLayout, { dynamicLayoutEnabled: false }),
    [homepageLayout]
  );
  ...
  useEffect(() => {
    ...
    if (!socket) return
    const handleStatusChange = () => { api.searchWorkers().then(r => setWorkers(r.data || [])) }
  ...
  const renderTileGrid = (serviceList) => (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 28 }}>
      {serviceList.map(service => (
        <ServiceTile key={service.role} service={service} isSelected={selectedCategory === service.role}
          onClick={() => { setStackedCategory(service.role); setSelectedCategory(service.role) }}
          cardStyle={cardStyle} iconStyle={iconStyle} labelStyle={{ ...labelStyle, fontSize: '11px' }} />
      ))}
    </div>
  )
    ...
    return (
      <div>
        ...
        <button onClick={() => { setStackedCategory(null); setSelectedCategory(null) }} style={{
          ...
          borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontSize: 'var(--font-body)',
          color: 'var(--text-secondary)', fontWeight: 500, padding: '6px 14px', marginBottom: 12, width: 'fit-content',
        }}>← {txt.back}</button>
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, overflowX: 'auto', paddingBottom: 8 }}>
          {categoryStack.map(service => (
            <CategoryButton key={service.role} service={service} isActive={stackedCategory === service.role}
              onClick={() => setStackedCategory(service.role)} />
          ))}
        </div>
        <h3 style={sectionTitleStyle}>{categories.find(c => c.role === stackedCategory)?.label || ''} — {txt.nearbyWorkers}</h3>
        {stackedWorkers.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-secondary)' }}>
            <div style={{ fontSize: 40, marginBottom: 8 }}>👷</div><p>{txt.noWorkers}</p>
          </div>
        ) : (
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder={txt.searchPlaceholder}
          style={{ flex: 1, minWidth: 200, padding: '12px 14px', borderRadius: 'var(--radius-md)',
        ...
        <select value={selectedLocation} onChange={e => setSelectedLocation(e.target.value)}
          style={{ padding: '12px 14px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)',
            background: 'var(--bg-surface2)', color: 'var(--text-primary)', fontSize: 'var(--font-body)',
      ...
      {showSearchResults && (
        <div style={{ marginBottom: 20 }}>
          {searching ? (
            ...
            <div style={{ textAlign: 'center', padding: 20, color: 'var(--text-secondary)' }}>No workers found</div>
          ) : (
              ...
              {searchResults.map(worker => (
                <div key={worker.id} onClick={() => navigate(`/detail/${worker.id}`)}
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--accent-blue)', fontWeight: 600, marginTop: 2 }}>{worker.client_id}</div>
                  </div>
      ...
      {/* Home content – resolver decides dynamic vs static */}
      {!showSearchResults && (
        <>
          {layoutDecision.useDynamic ? (
            <DynamicBlockRenderer blocks={layoutDecision.blocks} />
          ) : (
            <>
